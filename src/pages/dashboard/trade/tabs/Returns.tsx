...
import { ReturnState } from "../cruds/Return"

const Returns: React.FC = () => {
  const { isMobile } = useBreakpoints()
  const navigate = useNavigate()
  const toReturn = (state: ReturnState) => () => navigate("return", { state })

  const { activeTradePoint } = useTradePoints()
  const { dateRange, setDateRange, returns, getReturn, deleteReceipt, loading, error } = useTrade()
  const { head, details, currentPage, hasNext } = returns

  const fetchReturn = React.useCallback(
    (page: number) => {
      if (activeTradePoint) getReturn({ gtochkaid: activeTradePoint.gTochkaId, ...dateRange, page })
    },
  ...
  const [sentryRef] = useInfiniteScroll({
    loading,
    hasNextPage: hasNext,
    onLoadMore: () => fetchReturn(currentPage + 1),
    disabled: !!error
  })

  const [returnToDelete, setReturnToDelete] = React.useState<number | null>(null)
  const onSetReturnToDelete = (deleteId: number | null) => () => setReturnToDelete(deleteId)
  ...
  const actions: BottomSheetAction[] = [
    {
      title: "Редактировать",
      onClick: itemId => toReturn({ variant: "edit", detail: details.find(d => d.skladId === itemId) })()
    },
    {
      title: "Удалить",
      onClick: setReturnToDelete,
  ...
  const RenderNewButton: React.FC = React.useCallback(() => {
    return (
      <div className="px-4 sm:mr-auto">
        <Button className={isMobile ? "primary" : ""} onClick={toReturn({ variant: "new" })}>
          Новый возврат
        </Button>
      </div>
    )
  ...
  return (
    <div className="lg:mt-5 relative">
        ...
        <div className="sm:order-last">
          <div className="p-4  sm:shadow-shadow-1 rounded-md">
            <div className="flex items-center justify-between">
              ...
              <p className="volume-17 text-primary-blue">{head.cntReturn || 0}</p>
            </div>
            <div className="flex items-center justify-between mt-1">
              <p className="text-black volume-16 text-gray-2">На сумму:</p>
              <p className="volume-17 text-primary-blue">{head.sumReturn?.toFixed(2) || "0.00"}</p>
            </div>
          </div>
        </div>
            ...
            <div key={`sale_${index}`} className="flex mb-4">
              <div className="flex-1 border border-bg-separator grid grid-cols-4">
                <div className="col-span-3 p-2 border-r border-bg-separator">
                  <p
                    onClick={toReturn({ variant: "preview", detail: item })}
              ...
              {isMobile ? (
                ...
                <div className="flex flex-col px-2">
                  <button title="Редактировать" onClick={toReturn({ variant: "edit", detail: item })}>
                    <Edit className="text-primary-blue" />
                  </button>
                  <button className="mt-2" title="Удалить" onClick={onSetReturnToDelete(item.skladId)}>
      ...
      <Modal open={!!returnToDelete} onClose={onSetReturnToDelete(null)}>
        <DeleteModal
          label="возврат"
          onClose={onSetReturnToDelete(null)}
          deleteId={returnToDelete}
          onDelete={onDelete}
        />
      </Modal>
