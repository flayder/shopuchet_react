...
const Return: React.FC = () => {
  ...
  const { state } = useLocationWithState<ReturnState>()
  const { variant, detail } = state || {}
  const { activeTradePoint } = useTradePoints()
  const { loading, dateRange, getReturn, createReceipt, editReceipt, returnPaymentTabIndex, setReturnPaymentTabIndex } =
    useTrade()

  const goBack = () => navigate(-1)

  const { headerTitle } = useNavbars({
    headerTitle: variant === "new" ? "Новый возврат" : `Возврат №${detail?.recId}`
  })
  ...
  const onSubmit = (form: TradeForm) => {
    ...
    if (variant === "new")
      createReceipt(body).then(() => {
        getReturn({ gtochkaid: gTochkaId, ...dateRange, page: 0 })
        goBack()
      })
    else
              ...
              <FormProvider {...methods}>
                {isMobile ? (
                  <Modal open={showPaymentModal} onClose={toggleShowPaymentModal(false)}>
                    <p className="volume-22 text-center text-primary-blue">Возврат</p>

                    <Payment tabIndex={returnPaymentTabIndex} setTabIndex={setReturnPaymentTabIndex} />
